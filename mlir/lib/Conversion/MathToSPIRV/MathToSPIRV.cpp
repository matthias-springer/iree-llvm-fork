...
class CountLeadingZerosPattern final
  ...
  matchAndRewrite(math::CountLeadingZerosOp countOp, OpAdaptor adaptor,
    ...
    // We need to subtract from 31 given that the index ireturned by GLSL
    // FindUMsb is from the least significant bit. Theoretically this also
    // gives the correct result even if the integer has all zero bits, in
